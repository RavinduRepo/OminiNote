...
/// CPU). `onProgress(done, total)` fires per canvas as pages are appended, for
/// a live progress bar. (Perf 07/14/26.)
///
...
/// Any isolate failure falls back to an on-main export — a hiccup must never
/// fail the export (same guard the page-JSON offload uses).
Future<Uint8List> exportPdfInIsolate(
  ...
  for (final item in items) {
    data.add(await _serializeItem(item));
  }
  ...
  try {
    ...
    rp.close();
    return SyncfusionPdfExporter().exportTree(items, onProgress: onProgress);
  }
  ...
  rp.listen((msg) {
    ...
    if (m.containsKey('progress')) {
      onProgress?.call(m['progress'] as int, m['total'] as int);
